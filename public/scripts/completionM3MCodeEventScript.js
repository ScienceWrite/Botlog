...
(function () {
 ...
 async function getCodeForPlayer(playerId) {
  // Aktuelle Liste vergebener Codes laden
  const assignedCodesRaw = WA.state.assignedCodes || "{}";
  const assignedCodes = JSON.parse(assignedCodesRaw);
  
  // Schon einen Code? -> zurückgeben
  if (assignedCodes[playerId]) {
    return { code: assignedCodes[playerId], alreadyRegistered: true };
  }
  
  // Welche Codes sind noch frei?
  const usedCodes = new Set(Object.values(assignedCodes));
  const freeCode = CODE_POOL.find(c => !usedCodes.has(c));
  
  if (!freeCode) {
    return { code: null, alreadyRegistered: false, error: "no_codes_left" };
  }
  
  // Neuen Code eintragen
  assignedCodes[playerId] = freeCode;
  await WA.state.saveVariable("assignedCodes", JSON.stringify(assignedCodes));
  
  return { code: freeCode, alreadyRegistered: false };
}
